# =================================================================================================
#  Get-SystemHealthReport (Comprehensive System Checker) - PowerShell
# =================================================================================================
<#
.SYNOPSIS
...
.DESCRIPTION
    This script gathers a wide array of system data, including:
      ...
      • Attempted CPU temperature readings (if supported)

    It provides a structured overview of your system, suitable for diagnostics,
    audits, or troubleshooting. CPU temperature retrieval is limited by hardware
    and WMI support.

.NOTES
    ⚠️  Running this script with Administrator privileges is strongly recommended
    for full functionality, especially regarding:
        - Disk health (SMART)
        - Security software status
        - Event log analysis
    ...
    Author:   Your Name
    Version:  1.0
    Script:   Get-SystemHealthReport (Comprehensive System Checker) - PowerShell
#>
# =================================================================================================
...
if (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()
    ...
    Write-Warning "⚠️  It is recommended to run this script as Administrator for complete results."
    Write-Host "Some information (e.g., disk SMART, antivirus, detailed event logs) may be restricted."
    Start-Sleep -Seconds 2
}

# --------------------------- Script Start ---------------------------
Write-Host "`n🚀 Starting Comprehensive System Health Report..." -ForegroundColor Yellow
...
Get-CimInstance Win32_OperatingSystem | 
    Select-Object Caption, OSArchitecture, @{Name="InstallDate"; Expression={$_.InstallDate.ToString("yyyy-MM-dd HH:mm:ss")}}, Version, BuildNumber | Format-List

Write-Host "`n-- Computer Information --" -ForegroundColor Cyan
Get-CimInstance Win32_ComputerSystem | 
    Select-Object Name, Manufacturer, Model, Domain, UserName, TotalPhysicalMemory | Format-List
...
Get-CimInstance Win32_PhysicalMemory | 
    Select-Object DeviceLocator, Capacity, Speed, Manufacturer, PartNumber | Format-List


# =================================================================================================
#  Section 3: Disk Drives & Storage
# =================================================================================================
...
Get-CimInstance Win32_DiskDrive | 
    Select-Object Model, MediaType, Size, Partitions, Status | Format-List
...
try {
    ...
    if (-not $?) { Write-Warning "SMART status unavailable (WMI unsupported or insufficient privileges)." }
} catch {
...
Get-CimInstance Win32_NetworkAdapterConfiguration | Where-Object {$_.IPAddress} |
    Select-Object Description, IPAddress, IPSubnet, DefaultIPGateway, DNSHostName, MACAddress | Format-List
...
try {
    ...
    Write-Warning "Could not perform connectivity test: $($_.Exception.Message)"
}


# =================================================================================================
#  Section 5: CPU Temperature (Experimental)
# =================================================================================================
...
try {
    ...
    if ($tempSensor) {
        foreach ($sensor in $tempSensor) {
            ...
            Write-Host "🌡️  Current Temperature: $($celsius.ToString("F2")) °C / $($fahrenheit.ToString("F2")) °F" -ForegroundColor Green
        }
    } else {
        Write-Warning "No CPU temperature data via WMI (common on modern systems)."
        Write-Warning "Consider specialized hardware monitoring tools for accurate readings."
    }
} catch {
...
$criticalServices = @("wuauserv", "bits", "SvcHost", "Dnscache", "Dhcp")
...
Write-Host "`n-- Last 20 Errors from System Log (24 Hours) --" -ForegroundColor Cyan
try {
    ...
    Write-Warning "Could not retrieve System event logs: $($_.Exception.Message)"
}

Write-Host "`n-- Last 20 Errors from Application Log (24 Hours) --" -ForegroundColor Cyan
try {
    Get-WinEvent -LogName Application -MaxEvents 20 -ErrorAction SilentlyContinue |
        Where-Object {$_.LevelDisplayName -eq "Error" -and $_.TimeCreated -gt (Get-Date).AddHours(-24)} |
        Select-Object TimeCreated, Id, LevelDisplayName, ProviderName, Message | Format-Table -AutoSize -Wrap
} catch {
    Write-Warning "Could not retrieve Application event logs: $($_.Exception.Message)"
}
...
try {
    ...
    if ($antivirus) {
        foreach ($av in $antivirus) {
            Write-Host "🛡️  Product: $($av.displayName)"
        ...
        Write-Warning "Antivirus product info not found (SecurityCenter2 unavailable)."
    }
} catch {
...
try {
    ...
    if ($firewall) {
        ...
        Write-Warning "Firewall product info not found (SecurityCenter2 unavailable)."
    }
} catch {
...
#  Script End
# =================================================================================================
Write-Host "`n✅ System Health Report Complete!" -ForegroundColor Yellow
Write-Host "================================================================================" -ForegroundColor DarkGreen

# === Keep window open until manually closed ===
Write-Host "`nPress Enter to close this window, or just click the ❌ (top right)." -ForegroundColor Yellow
Read-Host
